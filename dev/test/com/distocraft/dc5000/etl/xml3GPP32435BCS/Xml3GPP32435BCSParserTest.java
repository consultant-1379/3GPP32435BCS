...
public class Xml3GPP32435BCSParserTest {
	 ...
	 * 
	 * try { getSeconds = Xml3GPP32435Parser.class.getDeclaredMethod("getSeconds",
	 * new Class[] {String.class}); handleTAGmoid =
	 * Xml3GPP32435Parser.class.getDeclaredMethod("handleTAGmoid", new Class[]
	 * {String.class}); calculateBegintime =
	 * Xml3GPP32435Parser.class.getDeclaredMethod("calculateBegintime", new Class[]
	 * {}); mainParserObject =
	 * Xml3GPP32435Parser.class.getDeclaredField("mainParserObject"); techPack =
	 * Xml3GPP32435Parser.class.getDeclaredField("techPack"); setType =
	 * Xml3GPP32435Parser.class.getDeclaredField("setType"); setName =
	 * Xml3GPP32435Parser.class.getDeclaredField("setName"); status =
	 * Xml3GPP32435Parser.class.getDeclaredField("status"); workerName =
	 * Xml3GPP32435Parser.class.getDeclaredField("workerName"); charValue =
	 * Xml3GPP32435Parser.class.getDeclaredField("charValue"); errorList =
	 * Xml3GPP32435Parser.class.getDeclaredField("errorList"); fileFormatVersion =
	 * Xml3GPP32435Parser.class.getDeclaredField("fileFormatVersion"); vendorName =
	 * Xml3GPP32435Parser.class.getDeclaredField("vendorName"); dnPrefix =
	 * Xml3GPP32435Parser.class.getDeclaredField("dnPrefix"); fsLocalDN =
	 * Xml3GPP32435Parser.class.getDeclaredField("fsLocalDN"); elementType =
	 * Xml3GPP32435Parser.class.getDeclaredField("elementType"); collectionBeginTime
	 * = Xml3GPP32435Parser.class.getDeclaredField("collectionBeginTime");
	 * measNameMap = Xml3GPP32435Parser.class.getDeclaredField("measNameMap");
	 * meLocalDN = Xml3GPP32435Parser.class.getDeclaredField("meLocalDN"); userLabel
	 * = Xml3GPP32435Parser.class.getDeclaredField("userLabel"); swVersion =
	 * Xml3GPP32435Parser.class.getDeclaredField("swVersion"); measInfoId =
	 * Xml3GPP32435Parser.class.getDeclaredField("measInfoId"); jobId =
	 * Xml3GPP32435Parser.class.getDeclaredField("jobId"); granularityPeriodDuration
	 * = Xml3GPP32435Parser.class.getDeclaredField("granularityPeriodDuration");
	 * granularityPeriodEndTime =
	 * Xml3GPP32435Parser.class.getDeclaredField("granularityPeriodEndTime");
	 * repPeriodDuration =
	 * Xml3GPP32435Parser.class.getDeclaredField("repPeriodDuration"); measIndex =
	 * Xml3GPP32435Parser.class.getDeclaredField("measIndex"); measValueIndex =
	 * Xml3GPP32435Parser.class.getDeclaredField("measValueIndex"); objectClass =
	 * Xml3GPP32435Parser.class.getDeclaredField("objectClass"); readVendorIDFrom =
	 * Xml3GPP32435Parser.class.getDeclaredField("readVendorIDFrom");
	 * fillEmptyMoidStyle =
	 * Xml3GPP32435Parser.class.getDeclaredField("fillEmptyMoidStyle");
	 * fillEmptyMoidValue =
	 * Xml3GPP32435Parser.class.getDeclaredField("fillEmptyMoidValue"); objectMask =
	 * Xml3GPP32435Parser.class.getDeclaredField("objectMask"); sourceFile =
	 * Xml3GPP32435Parser.class.getDeclaredField("sourceFile"); measObjLdn =
	 * Xml3GPP32435Parser.class.getDeclaredField("measObjLdn"); oldObjClass =
	 * Xml3GPP32435Parser.class.getDeclaredField("oldObjClass"); suspectFlag =
	 * Xml3GPP32435Parser.class.getDeclaredField("suspectFlag"); measFile =
	 * Xml3GPP32435Parser.class.getDeclaredField("measFile");
	 * 
	 * sourceFileC = SourceFile.class.getDeclaredConstructor(new Class[] {
	 * File.class, Properties.class, RockFactory.class, RockFactory.class,
	 * ParseSession.class, ParserDebugger.class, Logger.class });
	 * MeasurementFileImplC = MeasurementFileImpl.class.getDeclaredConstructor(new
	 * Class[] { SourceFile.class, String.class, String.class, String.class,
	 * String.class, Logger.class }); ParseSessionC =
	 * ParseSession.class.getDeclaredConstructor(new Class[] {long.class,
	 * Properties.class});
	 * 
	 * getSeconds.setAccessible(true); handleTAGmoid.setAccessible(true);
	 * calculateBegintime.setAccessible(true); mainParserObject.setAccessible(true);
	 * techPack.setAccessible(true); setType.setAccessible(true);
	 * setName.setAccessible(true); status.setAccessible(true);
	 * workerName.setAccessible(true); charValue.setAccessible(true);
	 * errorList.setAccessible(true); fileFormatVersion.setAccessible(true);
	 * vendorName.setAccessible(true); dnPrefix.setAccessible(true);
	 * fsLocalDN.setAccessible(true); elementType.setAccessible(true);
	 * collectionBeginTime.setAccessible(true); measNameMap.setAccessible(true);
	 * meLocalDN.setAccessible(true); userLabel.setAccessible(true);
	 * swVersion.setAccessible(true); measInfoId.setAccessible(true);
	 * jobId.setAccessible(true); granularityPeriodDuration.setAccessible(true);
	 * granularityPeriodEndTime.setAccessible(true);
	 * repPeriodDuration.setAccessible(true); measIndex.setAccessible(true);
	 * measValueIndex.setAccessible(true); objectClass.setAccessible(true);
	 * readVendorIDFrom.setAccessible(true); fillEmptyMoidStyle.setAccessible(true);
	 * fillEmptyMoidValue.setAccessible(true); objectMask.setAccessible(true);
	 * sourceFile.setAccessible(true); measObjLdn.setAccessible(true);
	 * oldObjClass.setAccessible(true); suspectFlag.setAccessible(true);
	 * measFile.setAccessible(true);
	 * 
	 * sourceFileC.setAccessible(true); MeasurementFileImplC.setAccessible(true);
	 * ParseSessionC.setAccessible(true);
	 * 
	 * //MeasurementFileImpl data =
	 * MeasurementFileImpl.class.getDeclaredField("data");
	 * 
	 * data.setAccessible(true);
	 * 
	 * // Main fileList = Main.class.getDeclaredField("fileList"); psession =
	 * Main.class.getDeclaredField("psession");
	 * 
	 * fileList.setAccessible(true); psession.setAccessible(true);
	 * 
	 * 
	 * // Field dMap = DataFormatCache.class.getDeclaredField("map"); //
	 * dMap.setAccessible(true); DataFormatCache.initialize(null);
	 * 
	 * ArrayList<DItem> al = new ArrayList(); DItem di1 = new DItem("DATETIME_ID",
	 * 1, "first", "pi"); DItem di2 = new DItem("filename", 2, "second", "pi");
	 * DItem di3 = new DItem("DIRNAME", 3, "third", "pi"); al.add(di1); al.add(di2);
	 * al.add(di3);
	 * 
	 * ArrayList<DItem> al2 = new ArrayList(); DItem di21 = new DItem("value", 1,
	 * "first", "pi"); al2.add(di21);
	 * 
	 * 
	 * DFormat df = new DFormat("tfid", "foldName", al); DFormat df2 = new
	 * DFormat("tfid", "foldName", al2); DataFormatCache dfc = new
	 * DataFormatCache(); HashMap hm = (HashMap) dMap.get(dfc); hm.put("if:tagID",
	 * df); hm.put("if2:tagID2", df2);
	 * 
	 * 
	...
	public void testStatus() {
		Xml3GPP32435BCSParser xml3gpp = new Xml3GPP32435BCSParser();
	...
	/*
	 * @Test public void testRun() { Xml3GPP32435Parser xml3gpp = new
	 * Xml3GPP32435Parser(); String homeDir = System.getProperty("user.home");
	 * Properties sprop = new Properties();
	 * sprop.setProperty("SessionHandling.storageFile", homeDir + File.separator +
	 * "storageFile"); sprop.setProperty("SessionHandling.log.types", "AdapterLog");
	 * sprop.setProperty("SessionHandling.log.AdapterLog.class",
	 * "com.distocraft.dc5000.common.AdapterLog");
	 * sprop.setProperty("SessionHandling.log.ADAPTER.inputTableDir", homeDir);
	 * Properties prop = new Properties(); prop.setProperty("baseDir", homeDir);
	 * prop.setProperty("interfaceName", "if2");
	 * prop.setProperty("x3GPPParser.vendorIDMask", "f.+(tagID2)");
	 * 
	 * // Creating file to be parsed HelpClass hc = new HelpClass(); ArrayList<File>
	 * al = new ArrayList<File>(); File x = hc.createFile(homeDir, "3GPPfile.xml",
	 * "<measValue measObjLdn=\"ffftagID2\">\n\t<measData>\n\t\t<measType p=\"key\">value</measType>\n"
	 * + "\t\t<r p=\"key\">rvalue</r>\n\t</measData>\n</measValue>"); al.add(x);
	 * x.deleteOnExit(); File out = new File(homeDir, "out"); out.mkdir();
	 * 
	 * // Initializing transformer cache TransformerCache tc = new
	 * TransformerCache();
	 * 
	 * try { MeasurementFileImpl.setTestMode(true); ParseSession ps = (ParseSession)
	 * ParseSessionC.newInstance(new Object[] {1L, null}); Main main = new
	 * Main(prop, "x", "x", "x", null, null, new EngineCom()); fileList.set(main,
	 * al); psession.set(main, ps);
	 * 
	 ...
	 * 
	 * // Calling the tested method xml3gpp.run();
	 * 
	 * File tp = new File(homeDir + File.separator + "out\\tp"); File i = new
	 * File(homeDir + File.separator + "out\\tp\\foldName_wn_null");
	 * 
	 * String actual = hc.readFileToString(i); String expected = "rvalue\t";
	 * 
	 * MeasurementFileImpl mf = (MeasurementFileImpl) measFile.get(xml3gpp);
	 * mf.close(); // Should this be in endElement method
	 * 
	 * i.delete(); tp.delete(); out.delete();
	 * 
	 * assertEquals(expected, actual);
	 * 
	 * } catch(Exception e) { e.printStackTrace(); fail("testRun() failed"); } }
	 */
...
//  }
	...
	/*
	 * @Test public void testStartElementStringStringStringAttributes7() {
	 * Xml3GPP32435Parser xml3gpp = new Xml3GPP32435Parser();
	 * 
	 * AttributesImpl atts = new AttributesImpl(); atts.addAttribute("uri", "jobId",
	 * "jobId", "type", "jid");
	 * 
	 * try { Calling the tested method xml3gpp.startElement(null, null, "job",
	 * atts);
	 * 
	 * String actual = ""; String expected = "jid"; actual += jobId.get(xml3gpp);
	 * 
	 * assertEquals(expected, actual);
	 * 
